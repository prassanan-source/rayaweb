...
def place_kitchen_order(inp: dict, square) -> dict:
    ...
    if inp.get("diningOption") == "delivery":
        return {
            "ok": False,
            "code": "SQUARE_DELIVERY_REQUIRES_ONLINE",
            "error": (
                "Square On-Demand Delivery must be placed through the published "
                "Square Online store; the Orders API cannot dispatch a courier."
            ),
        }
